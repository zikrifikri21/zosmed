...
class InviteUsersRequest extends FormRequest
{
    ...
    public ?GroupUsers $groupUsers = null;
        ...
        return [
            'email' => ['required', function($attribute, $value, Closure $fail) {
               $this->user = User::query()->where('email', $value)
                ...
                $this->groupUsers = GroupUsers::where('user_id', $this->user->id)
                    ->where('group_id', $this->group->id)
                    ->first();

                if($this->groupUsers && $this->groupUsers->status === GroupUserStatus::APPROVED->value){
                    $fail('You are already a member of this group');
                }
            }]
        ];
    }
